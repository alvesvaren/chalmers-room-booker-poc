 ...
 */
export function useAuthFailureInterceptor(token: string, logOut: LogOut, onSessionToast: (message: string) => void) {
    ...
    const id = client.interceptors.response.use(async (response, request) => {
      ...
      if (response.status === 401 && !isLogin && hadToken) {
        if (!authFailureHandled.current) {
          ...
          logOut();
          onSessionToast("Sessionen har gått ut eller är ogiltig. Logga in igen.");
        }
        return response;
      }
      ...
      if (response.status === 502 && !isLogin && hadToken) {
        ...
        if (body?.error === TIMEEDIT_AUTH_FAILED_ERROR && !authFailureHandled.current) {
          authFailureHandled.current = true;
          logOut();
          onSessionToast("TimeEdit accepterade inte inloggningen. Logga in igen.");
        }
      }
